...
/datum/sex_action/titjob/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(get_breast_piercing(target))
		user.visible_message(span_warning("[user] grabs [target]'s pierced tits and shoves [user.p_their()] cock inbetween!"))
	else
...
/datum/sex_action/titjob/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(get_breast_piercing(target))
		user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] fucks [target]'s tits while tweaking and teasing their pierced nipples."))
	else
