...
/datum/sex_action/force_titjob/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(get_breast_piercing(user))
		user.visible_message(span_warning("[user] grabs [target]'s cock and shoves it between [user.p_their()] pierced tits!"))
	else
...
/datum/sex_action/force_titjob/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	if(get_breast_piercing(user))
		user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] shoves [target]'s cock between [user.p_their()] tits, dragging it along [user.p_their()] nipple piercings."))
	else
