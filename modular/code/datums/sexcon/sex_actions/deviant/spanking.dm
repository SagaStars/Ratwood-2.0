...
/datum/sex_action/spanking/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
	...
	var/sound_chaste = pick('sound/foley/slap.ogg', 'sound/foley/smackspecial.ogg', 'sound/items/jinglebell1.ogg')
	playsound(target, sound, 50, TRUE, -2, ignore_walls = FALSE)
	if(target.sexcon.has_chastity_cage())
		playsound(target, sound_chaste, 50, TRUE, -2, ignore_walls = FALSE)
