...
/datum/sex_action/chastityplay/force_cage_blowjob/on_start(mob/living/carbon/human/user, mob/living/carbon/human/target)
    user.visible_message(span_warning("[user] forces [target]'s face into [user.p_their()] [get_chastity_device_name(user)]!"))

/datum/sex_action/chastityplay/force_cage_blowjob/on_perform(mob/living/carbon/human/user, mob/living/carbon/human/target)
    user.visible_message(user.sexcon.spanify_force("[user] [user.sexcon.get_generic_force_adjective()] keeps [target]'s mouth pressed to [user.p_their()] [get_chastity_device_name(user)]."))
