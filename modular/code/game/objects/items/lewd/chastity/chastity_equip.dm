...
/obj/item/chastity/attack(mob/M, mob/user, def_zone) // equipping others with chastity device
    ...
    if(chastity_cursed)
        ...
        if(!CM || !CM.add_pet(H))
            to_chat(user, span_warning("The cursed device fails to recognize its master and slips loose."))
            remove_chastity(H)
...
/obj/item/chastity/proc/attempt_forced_removal(mob/living/carbon/human/H, mob/user)
	...
	if(is_hardmode_active())
		to_chat(user, span_warning("Your tools scrape uselessly against enchanted metal. This binding will not break."))
		playsound(src, 'sound/items/pickbad.ogg', 40, TRUE)
		return FALSE

	var/obj/item/I = user.get_active_held_item()
	if(!I || !istype(I, /obj/item/rogueweapon/chisel))
		to_chat(user, span_warning("I need a chisel to attempt this."))
		return FALSE

	to_chat(user, span_warning("I begin attempting to chisel [H]'s chastity device open. This will be painful and dangerous."))
	user.visible_message(span_danger("[user] begins chiseling at [H]'s chastity device!"))

	if(!do_after(user, 100, needhand = TRUE, target = H))
		to_chat(user, span_warning("I stop my attempt."))
		return FALSE

	var/success_chance = 25
	if(ishuman(user))
		var/mob/living/carbon/human/U = user
		var/craft_skill = U.get_skill_level(/datum/skill/craft/blacksmithing) || 0
		success_chance += craft_skill * 10
...
/obj/item/chastity/Destroy() // failsafe to remove chastity traits if the belt get's Qdel'd or something
	var/mob/living/carbon/human/old_wearer = chastity_victim
	detach_toy()
	if(old_wearer)
		remove_chastity(old_wearer)
	generated_key = null
	chastity_feature = null
	chastity_victim = null
	chastity_master = null
	chastity_move_counter = 0
	return ..()
